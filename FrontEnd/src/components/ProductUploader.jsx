...
export default function ProductUploader({ productImages, setProductImages, isGenerating, setGlobalError }) {
      ...
      {productImages.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", marginTop: "2rem" }}>
          {productImages.map((img) => (
            <div 
              key={img.id} 
              style={{ position: "relative", width: "140px", height: "140px", borderRadius: "var(--radius-md)", overflow: "hidden", border: "2px solid var(--aurora-border)", boxShadow: "var(--shadow-sm)" }}
            >
              <img src={img.previewUrl} alt={img.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              <button 
                onClick={() => removeProductImage(img.id)} 
                disabled={isGenerating} 
                style={{ position: "absolute", top: "6px", right: "6px", background: "rgba(0,0,0,0.7)", color: "white", border: "none", borderRadius: "50%", width: "28px", height: "28px", cursor: "pointer", transition: "background 0.2s" }}
              >
                ✕
              </button>
            </div>
