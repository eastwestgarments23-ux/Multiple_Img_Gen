...
export default function ResultsMatrix({ generationResults, isGenerating, productImages, selectedModelId }) {
  ...
  if (generationResults.length === 0 && !isGenerating) return null;
  ...
  return (
    <div className="glass-panel" style={{ padding: "2.5rem", borderRadius: "var(--radius-lg)" }}>
      ...
      {productImages.map((prodImg) => {
        ...
        return (
          <div key={prodImg.id} style={{ marginBottom: "2.5rem", padding: "1.5rem", background: "white", borderRadius: "var(--radius-lg)", border: "1px solid var(--aurora-border)", boxShadow: "var(--shadow-sm)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", paddingBottom: "1.5rem", borderBottom: "2px dashed var(--aurora-border)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "1.25rem" }}>
                <img src={prodImg.previewUrl} alt={prodImg.name} style={{ width: "60px", height: "60px", borderRadius: "var(--radius-sm)", objectFit: "cover", border: "1px solid var(--aurora-border)" }} />
                <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "1rem" }}>
              {specificResults.map((res) => (
                <div key={res.uid} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", padding: "0.5rem", border: "1px solid var(--aurora-border)", borderRadius: "var(--radius-md)", background: "var(--aurora-bg)" }}>
                  <div style={{ width: "100%", aspectRatio: "3/4", background: "#f8fafc", borderRadius: "var(--radius-sm)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative" }}>
                    {res.status === "loading" && (
                    ...
                    {res.status === "success" && <img src={res.blobUrl} alt={`Pose ${res.poseId}`} style={{ width: "100%", height: "100%", objectFit: "contain" }} />}
                    {res.status === "error" && (
                  ...
                  {res.status === "success" && (
                    <button onClick={() => triggerDownload(res.blobUrl, res.filename)} className="aurora-btn aurora-btn-outline" style={{ padding: "0.5rem", fontSize: "0.85rem" }}>
