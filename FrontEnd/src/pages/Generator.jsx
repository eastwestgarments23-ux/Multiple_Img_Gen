...
import { Upload, Image as ImageIcon, Filter, CheckCircle, Loader2, Download, Package, AlertCircle, Zap, ArrowUpRight } from 'lucide-react';
...
export default function Generator({ user }) {
    ...
    files.forEach((file, index) => {
      ...
      reader.onload = (event) => {
        ...
        setProductImages((prev) => [
          ...prev, 
          { id: Date.now() + index, name: file.name.split('.')[0], mimeType: file.type, data: base64String, previewUrl: event.target.result }
        ]);
      };
  ...
  const removeProductImage = (idToRemove) => setProductImages(prev => prev.filter(img => img.id !== idToRemove));
  ...
  const convertBase64ToBlob = (base64Data, mimeType = 'image/png') => {
    ...
    for (let i = 0; i < rawBinary.length; i++) bytesBuffer[i] = rawBinary.charCodeAt(i);
  ...
  const handleGenerate = async () => {
    ...
    productImages.forEach(prodImg => {
      targetModel.poses.forEach(pose => {
        matrixState.push({
          uid: `${prodImg.id}-${pose.id}`, productId: prodImg.id, productName: prodImg.name,
          modelId: targetModel.id, poseId: pose.id, status: 'loading',
          blobUrl: null, blob: null, filename: `Model_${targetModel.id}_Pose_${pose.id}_${prodImg.name}.png`, errorMsg: null
        });
      });
    });
      ...
      for (const pose of targetModel.poses) {
        ...
        try {
          ...
          const response = await fetch('/api/generate-pose', {
            ...
            body: JSON.stringify({
              base64Image: prodImg.data, mimeType: prodImg.mimeType, sourceName: prodImg.name,
              poseBase64: poseBase64Data, poseMimeType: 'image/jpeg',
              modelId: targetModel.id, poseId: pose.id, ethnicity: targetModel.ethnicity
            })
          });
          ...
          if (!response.ok) {
            ...
            if (data.error === "LIMIT_REACHED") {
                setLimitData({ currentTier: data.currentTier || 'free', message: data.message });
                haltExecution = true;
          ...
          setGenerationResults(prev => prev.map(res => res.uid === uid ? { ...res, status: 'success', blobUrl, blob: imageBlob } : res));

        } catch (err) {
          if (err.message !== "Quota Exceeded") {
            setGenerationResults(prev => prev.map(res => res.uid === uid ? { ...res, status: 'error', errorMsg: err.message } : res));
          } else {
             // Mark all remaining loading states as cancelled
             setGenerationResults(prev => prev.map(res => res.status === 'loading' ? { ...res, status: 'error', errorMsg: 'Cancelled due to limit' } : res));
          }
        }
      }
    }
    ...
    try {
        ...
        const options = {
            ...
            handler: async function (response) {
                ...
                if (verifyRes.ok) {
                    alert("Payment Successful! Your tier has been upgraded. Please try generating again.");
  ...
  const displayModels = AVAILABLE_MODELS.filter(m => (ethnicityFilter === 'all' || m.ethnicity === ethnicityFilter) && !brokenModels.has(m.id));
    ...
    <div style={{ padding: '3rem 5%', maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
      ...
      {limitData && TIERS[limitData.currentTier]?.next && (
        <div style={{ 
            ...
            color: 'white', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '2rem',
            boxShadow: '0 20px 25px -5px rgba(67, 56, 202, 0.3)'
        }}>
            <div style={{ flex: '1 1 300px' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,255,255,0.2)', padding: '0.25rem 0.75rem', borderRadius: 'var(--radius-full)', fontSize: '0.85rem', fontWeight: 600, marginBottom: '1rem' }}>
                    <AlertCircle size={16} /> Daily Limit Reached
                </div>
                <h2 style={{ fontSize: '1.75rem', fontWeight: 700, margin: '0 0 0.5rem 0' }}>Upgrade to {TIERS[limitData.currentTier].name}</h2>
                <p style={{ margin: 0, opacity: 0.9, lineHeight: 1.5 }}>
                    You've hit your {limitData.currentTier} plan limit. Upgrade now to generate up to <strong>{TIERS[limitData.currentTier].limit} images per day</strong> and keep your workflow moving without interruption.
                </p>
            </div>
            <button 
                ...
                style={{ background: 'white', color: '#4338ca', padding: '1rem 2rem', fontSize: '1.1rem', whiteSpace: 'nowrap' }}
            >
                {isProcessingPayment ? <Loader2 className="spin" size={20} /> : <Zap size={20} />}
                {isProcessingPayment ? "Processing..." : `Upgrade for ${TIERS[limitData.currentTier].price}`}
            </button>
        </div>
      )}
      ...
      <div className="glass-panel" style={{ padding: '2.5rem', borderRadius: 'var(--radius-lg)', marginBottom: '2rem' }}>
        ...
        <button onClick={() => fileInputRef.current?.click()} className="aurora-btn aurora-btn-outline" style={{ padding: '1rem 2rem', fontSize: '1.1rem' }} disabled={isGenerating}>
      ...
      <div className="glass-panel" style={{ padding: '2.5rem', borderRadius: 'var(--radius-lg)', marginBottom: '2.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem', borderBottom: '2px solid var(--aurora-border)', paddingBottom: '1.5rem', marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <select className="aurora-input" style={{ width: 'auto', padding: '0.75rem 2.5rem 0.75rem 1rem', cursor: 'pointer' }} value={ethnicityFilter} onChange={(e) => { setEthnicityFilter(e.target.value); setSelectedModelId(null); }} disabled={isGenerating}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          {displayModels.map(model => (
            <div key={model.id} style={{ border: `3px solid ${selectedModelId === model.id ? 'var(--aurora-primary)' : 'transparent'}`, background: selectedModelId === model.id ? 'rgba(79, 70, 229, 0.04)' : 'var(--aurora-surface)', borderRadius: 'var(--radius-lg)', padding: '1.5rem', cursor: isGenerating ? 'not-allowed' : 'pointer', opacity: isGenerating && selectedModelId !== model.id ? 0.4 : 1, boxShadow: selectedModelId === model.id ? 'var(--shadow-md)' : 'var(--shadow-sm)' }} onClick={() => !isGenerating && setSelectedModelId(model.id)}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem' }}>
                {model.poses.map(pose => (
                  <div key={pose.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
                    <img src={pose.src} alt={`Pose ${pose.id}`} onError={() => handleImageError(model.id)} style={{ width: '130px', height: '180px', objectFit: 'cover', borderRadius: 'var(--radius-md)', border: '2px solid var(--aurora-border)', background: '#fff', boxShadow: 'var(--shadow-sm)' }} />
      ...
      <button className="aurora-btn aurora-btn-primary" style={{ width: '100%', padding: '1.25rem', fontSize: '1.25rem', marginBottom: '2.5rem', borderRadius: 'var(--radius-lg)' }} onClick={handleGenerate} disabled={isGenerating || !selectedModelId || productImages.length === 0}>
        {isGenerating ? <><Loader2 size={28} className="spin" style={{ animation: 'spin 1s linear infinite' }} /><span>Processing Matrix...</span></> : <><CheckCircle size={28} /><span>Generate Outputs</span></>}
      </button>
      ...
      {(generationResults.length > 0 || isGenerating) && (
        <div className="glass-panel" style={{ padding: '2.5rem', borderRadius: 'var(--radius-lg)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1.5rem' }}>
            <h3 style={{ fontSize: '1.5rem', margin: 0, display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--aurora-primary)' }}><Package size={28} />Generated Results</h3>
            {generationResults.some(r => r.status === 'success') && <button onClick={downloadMasterZip} className="aurora-btn aurora-btn-outline" style={{ background: 'white', padding: '0.75rem 1.25rem' }}>📦 Download Master ZIP</button>}
          </div>
              ...
              <div key={prodImg.id} style={{ marginBottom: '2.5rem', padding: '2rem', background: 'white', borderRadius: 'var(--radius-lg)', border: '1px solid var(--aurora-border)', boxShadow: 'var(--shadow-sm)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', paddingBottom: '1.5rem', borderBottom: '2px dashed var(--aurora-border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                    ...
                    <div><div style={{ fontSize: '0.85rem', color: 'var(--aurora-text-muted)', fontWeight: 700 }}>SOURCE</div><div style={{ fontWeight: 600, fontSize: '1.1rem' }}>{prodImg.name}</div></div>
                  </div>
                  {specificResults.some(r => r.status === 'success') && <button onClick={() => downloadArrayZip(prodImg.id, prodImg.name)} className="aurora-btn" style={{ background: 'var(--aurora-text-main)', color: 'white', padding: '0.6rem 1.25rem' }}>⬇️ Array .ZIP</button>}
                </div>
                  ...
                  {specificResults.map(res => (
                    <div key={res.uid} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '0.75rem', border: '1px solid var(--aurora-border)', borderRadius: 'var(--radius-md)', background: 'var(--aurora-bg)' }}>
                      <div style={{ width: '100%', aspectRatio: '3/4', background: '#e2e8f0', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
                        {res.status === 'loading' && <div style={{ textAlign: 'center', color: 'var(--aurora-text-muted)' }}><Loader2 size={32} style={{ animation: 'spin 1s linear infinite', margin: '0 auto 0.75rem' }} /><div style={{ fontSize: '0.9rem', fontWeight: 600 }}>Pose {res.poseId}</div></div>}
                        {res.status === 'success' && <img src={res.blobUrl} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                        {res.status === 'error' && <div style={{ textAlign: 'center', color: 'var(--aurora-danger)', padding: '1rem' }}><AlertCircle size={28} style={{ margin: '0 auto 0.5rem' }} /><div style={{ fontSize: '0.8rem', fontWeight: 600 }}>{res.errorMsg}</div></div>}
                      </div>
                      {res.status === 'success' && <button onClick={() => triggerDownload(res.blobUrl, res.filename)} className="aurora-btn aurora-btn-outline" style={{ padding: '0.6rem' }}><Download size={16} /> Download</button>}
                    </div>
                  ))}
                </div>
              </div>
