...
import { User, Mail, Phone, Calendar, Image as ImageIcon, Loader2, AlertCircle, Clock, ChevronLeft, ChevronRight, Key, Coins, CheckCircle2, Trash2, X } from 'lucide-react';
...
export default function Profile({ user }) {
  ...
  return (
    <>
      ...
      {/* Themed Alert Modal for Deletion Confirmation */}
      {showDeleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', padding: '1rem' }}>
          <div className="glass-panel" style={{ background: 'white', padding: '2.5rem', borderRadius: 'var(--radius-lg)', maxWidth: '450px', width: '100%', borderTop: `4px solid var(--aurora-danger)`, boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)', position: 'relative' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '1rem' }}>
              <div style={{ background: '#fef2f2', padding: '1rem', borderRadius: '50%' }}>
                <AlertCircle size={40} style={{ color: 'var(--aurora-danger)' }} />
              </div>
              <h3 style={{ margin: 0, fontSize: '1.35rem', fontWeight: 700 }}>Delete API Key?</h3>
              <p style={{ margin: 0, color: 'var(--aurora-text-muted)', lineHeight: 1.5, fontSize: '0.95rem' }}>
                Are you sure you want to delete your Google Gemini API key? You will not be able to generate new images until you provide a new key.
              </p>
              
              <div style={{ display: 'flex', gap: '1rem', width: '100%', marginTop: '1.5rem' }}>
                <button onClick={() => setShowDeleteConfirm(false)} className="aurora-btn aurora-btn-outline" style={{ flex: 1, justifyContent: 'center', background: 'white' }}>
                  Cancel
                </button>
                <button onClick={executeDeleteApiKey} className="aurora-btn" style={{ flex: 1, justifyContent: 'center', background: 'var(--aurora-danger)', color: 'white' }}>
                  Yes, Delete Key
                </button>
              </div>
            </div>
