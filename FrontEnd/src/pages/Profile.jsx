...
import { User, Mail, Phone, Calendar, Image as ImageIcon, Loader2, AlertCircle, Zap, Clock } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        // Fetch User Profile & Usage Stats
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: 'var(--radius-lg)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ padding: '0.5rem', background: '#f1f5f9', borderRadius: 'var(--radius-md)', color: 'var(--aurora-text-muted)' }}>
                <Mail size={16} />
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--aurora-text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Email Address</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--aurora-text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Phone Number</div>
                <div style={{ fontWeight: 500 }}>{userData.phone}</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--aurora-text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Member Since</div>
                <div style={{ fontWeight: 500 }}>{new Date(userData.created_at).toLocaleDateString()}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: 'var(--radius-lg)' }}>
          ...
          <div style={{ marginBottom: '1rem' }}>
            ...
            <div style={{ width: '100%', height: '8px', background: 'var(--aurora-border)', borderRadius: 'var(--radius-full)', overflow: 'hidden' }}>
              <div style={{ 
                ...
                background: usagePercentage >= 100 ? 'var(--aurora-danger)' : 'var(--aurora-primary)',
          ...
          <p style={{ fontSize: '0.875rem', color: 'var(--aurora-text-muted)', lineHeight: 1.5 }}>
            {usage.remaining > 0 
              ? `You have ${usage.remaining} generation${usage.remaining === 1 ? '' : 's'} remaining for today. Your limit resets at midnight.` 
              : `You have reached your daily limit of ${usage.limit} generations. Please check back tomorrow!`}
          </p>
        </div>
      </div>
              ...
              <tbody>
                {galleryHistory.map((item) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--aurora-border)' }}>
                    <td style={{ padding: '1rem 0.5rem', fontWeight: 500, color: 'var(--aurora-primary)', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {item.file_name}
                    </td>
                    <td style={{ padding: '1rem 0.5rem' }}>
                      <span style={{ background: '#f1f5f9', padding: '0.25rem 0.5rem', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem', fontWeight: 500 }}>
                        {item.parent_folder.replace(/_/g, ' ')}
                      </span>
                    </td>
                    <td style={{ padding: '1rem 0.5rem', color: 'var(--aurora-text-muted)', fontSize: '0.9rem' }}>
